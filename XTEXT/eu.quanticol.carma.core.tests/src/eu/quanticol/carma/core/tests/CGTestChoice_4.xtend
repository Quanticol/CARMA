...
component Producer(){
    ...
    behaviour{
        Produce = [my.product > 0] produce*{product := product + 1}.Send + [my.product == 0] produceDouble*{product := product + 2}.Send;
        Send = [my.product > 0] send*{product := product - 1}.Produce;
    }
...
public class __synthetic0Definition {
	...
	private static CarmaProcessAutomaton createProducerProcess() {
		...
		toReturn.addTransition(state_Send,Send_Guard,send_Action,state_Produce);
		toReturn.addTransition(state_Produce,Produce_Guard,produceDouble_Action,state_Send);
		toReturn.addTransition(state_Produce,Produce_Guard,produce_Action,state_Send);
